...
class EEGPreImageDataset(Dataset):
    def __init__(
        ...
        if self.image_aug:
            ...
            for aug_image_feature_dir in aug_image_feature_dirs:
                if train:
                    aug_image_feature_path = os.path.join(aug_image_feature_dir, "train.npy") #(1654, 10, 1024)
                else:
                    aug_image_feature_path = os.path.join(aug_image_feature_dir, "test.npy")
                
                #logger.info(f"aug_image_feature_path: {len(aug_image_feature_path)}")
    ...
    def __getitem__(self, index):
        ...
        if self.image_aug:
            #logger.info(f"aug_image_feature: {len(self.aug_image_features)}")
            if self.train or self.image_test_aug:
                ...
                image_feature = np.stack(
                    [a[0, object_idx, image_idx, :] for a in self.aug_image_features],
                    axis=0
                )
                #self.aug_image_features = np.stack(self.aug_image_features, axis=0)
                #image_feature = self.aug_image_features[:,0,object_idx,image_idx]
                #print(image_feature.shape)
            else:
                image_feature = self.image_features[object_idx][image_idx]
        else:
            image_feature = self.image_features[object_idx][image_idx]
        #logger.info(f"image_feature: {image_feature.shape}")
